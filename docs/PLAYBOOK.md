...
1. Run `python -m archive_mcp index-status`.
...
4. Run `archive_embed_pending` through MCP or `python -m archive_mcp embed-pending` to fill pending chunks for the configured provider.
...
3. Re-run `python -m archive_mcp rebuild-indexes`.
4. Re-run `python -m archive_mcp index-status`.
