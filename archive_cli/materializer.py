...
from archive_vault.schema import BaseCard
from archive_vault.vault import extract_wikilinks, read_note_file
    ...
    *,
    ...
    if ppa_engine() == "rust":
        try:
            ...
            return archive_crate.materialize_row_batch(
                rows,
                vault_root,
                slug_map,
                path_to_uid,
            ...
            warnings.warn(
                f"PPA: falling back to Python materializer — archive_crate error: {e}",
                stacklevel=2,
            )
    batch = ProjectionRowBuffer()
    for row in rows:
