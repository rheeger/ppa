...
def _vault_paths_and_fingerprint_python(vault: Path, rel_paths: list[str]) -> tuple[dict[str, tuple[int, int]], str]:
    lines: list[str] = []
...
def _vault_paths_and_fingerprint(vault: Path, rel_paths: list[str]) -> tuple[dict[str, tuple[int, int]], str]:
    ...
    if ppa_engine() == "rust":
        try:
            ...
            stats_obj, fp = archive_crate.vault_paths_and_fingerprint(str(vault), rel_paths)
            ...
            warnings.warn(
                f"PPA: falling back to Python for vault_paths_and_fingerprint — archive_crate not available: {e}",
                stacklevel=2,
            )
    return _vault_paths_and_fingerprint_python(vault, rel_paths)
...
def cards_by_type_from_cache_path(
    cache_path: Path | str,
    types: list[str] | None = None,
) -> dict[str, list[str]]:
    """Return ``card_type -> rel_paths`` from a Tier-1/2 vault scan SQLite cache.

    Uses :func:`archive_crate.cards_by_type_from_cache` when the extension is built;
    otherwise queries SQLite directly (same schema as :class:`archive_cli.vault_cache.VaultScanCache`).
    """
    ...
    try:
        ...
        return dict(archive_crate.cards_by_type_from_cache(str(cache_path), types))
    except (ImportError, Exception) as e:
        warnings.warn(
            f"PPA: falling back to Python for cards_by_type_from_cache — archive_crate not available: {e}",
            stacklevel=2,
        )
    ...
    try:
        if types:
            ...
            rows = conn.execute(
                f"SELECT card_type, rel_path FROM notes WHERE card_type IN ({placeholders}) ORDER BY card_type, rel_path",
                tuple(types),
            ).fetchall()
        else:
            rows = conn.execute(
    ...
    for ct, rp in rows:
        out.setdefault(str(ct), []).append(str(rp))
    return out
