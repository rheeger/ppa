...
from archive_cli.index_store import (
...
def test_classify_unchanged_noop() -> None:
    ...
    from archive_cli.index_store import _frontmatter_hash_stable
...
def test_content_hash_detects_body_change_same_mtime(tmp_path: Path) -> None:
    ...
    with patch("archive_cli.scanner.read_note_file", return_value=fake_note):
        mode, mat, purge, _ = _classify_manifest_rebuild_delta(
            [row],
