...
from archive_cli.benchmark import (
    BENCHMARK_PROFILES,
    benchmark_rebuild,
...
from archive_vault.schema import CalendarEventCard, EmailMessageCard, EmailThreadCard, MeetingTranscriptCard, PersonCard
...
def live_archive(
    ...
    monkeypatch.setattr("archive_cli.store.get_embedding_provider", _fixture_embedding_provider)
    monkeypatch.setattr(
...
def test_live_postgres_rebuild_graph_and_lexical_search(live_archive):
    ...
    assert index.status()["chunk_schema_version"] == str(CHUNK_SCHEMA_VERSION)
...
def test_benchmark_seed_links_returns_metrics(live_archive, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "archive_cli.benchmark.run_seed_link_backfill",
        lambda idx, **kwargs: {
    ...
    monkeypatch.setattr(
        "archive_cli.benchmark.compute_link_quality_gate",
        lambda idx: {"passes": True, "scan_coverage": 1.0},
    )
    monkeypatch.setattr(
        "archive_cli.benchmark._cleaning_snapshot",
        lambda vault: {"email_messages_with_thread": 1, "orphaned_wikilinks": 2},
    )
    monkeypatch.setattr(
        "archive_cli.benchmark._repair_opportunities",
        lambda index: [
            {
