...
def test_archive_bootstrap_postgres_uses_postgres_backend(tmp_vault, monkeypatch: pytest.MonkeyPatch):
    ...
    def fake_bootstrap(self, *, force: bool = False) -> dict[str, str]:
...
def test_postgres_bootstrap_command_dispatches(
    ...
    from archive_cli.commands import admin as admin_cmd
    ...
    monkeypatch.setattr(sys, "argv", ["archive_cli", "bootstrap-postgres"])
    archive_main.main()
...
def test_postgres_embed_pending_command_dispatches(
    ...
    monkeypatch.setattr(sys, "argv", ["archive_cli", "embed-pending"])
    archive_main.main()
...
def test_seed_link_commands_dispatch(
    ...
    from archive_cli.commands import status as status_cmd
    ...
    monkeypatch.setattr(sys, "argv", ["archive_cli", "seed-link-surface"])
    archive_main.main()
    ...
    monkeypatch.setattr(sys, "argv", ["archive_cli", "seed-link-enqueue", "--job-type", "seed_backfill"])
    archive_main.main()
    ...
    monkeypatch.setattr(sys, "argv", ["archive_cli", "seed-link-backfill", "--workers", "3"])
    archive_main.main()
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["archive_cli", "seed-link-refresh", "--source-uids", "hfa-person-1"],
    )
    archive_main.main()
    ...
    monkeypatch.setattr(sys, "argv", ["archive_cli", "seed-link-worker", "--workers", "2"])
    archive_main.main()
    ...
    monkeypatch.setattr(sys, "argv", ["archive_cli", "seed-link-promote", "--workers", "2"])
    archive_main.main()
...
def test_benchmark_seed_links_command_dispatches(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["archive_cli", "benchmark-seed-links", "--vault", str(tmp_path / "vault")],
    )
    archive_main.main()
...
def test_openai_embedding_provider_parses_embedding_response(
    ...
    monkeypatch.setattr("archive_cli.embedding_provider._post_json", fake_post_json)
...
def test_resolve_openai_api_key_from_arnold_vault_toggle(
    ...
    monkeypatch.setattr(
        "archive_cli.embedding_provider._read_1password_secret",
...
def test_resolve_service_account_token_prefers_passkey_gate_op_ref(
    ...
    monkeypatch.setattr(
        "archive_cli.embedding_provider._run_op_read",
...
def test_resolve_service_account_token_falls_back_to_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    monkeypatch.setattr("archive_cli.embedding_provider._run_op_read", lambda reference, env: None)
