...
def test_positive_match_rate(positive_pairs):
    ...
    assert rate >= POSITIVE_GATE, (
        f"positive match rate {hits}/{len(positive_pairs)}={rate:.2%} below gate {POSITIVE_GATE:.0%}"
    )
...
def test_negative_reject_rate(negative_pairs):
    ...
    assert rate >= NEGATIVE_GATE, (
        f"negative true-reject rate {correct}/{len(negative_pairs)}={rate:.2%} below gate {NEGATIVE_GATE:.0%}"
    )
