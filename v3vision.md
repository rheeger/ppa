...
**Files touched (in addition to those listed above):** `archive_mcp/vault_cache.py` (type-filtered loading, per-directory fingerprints, progress callbacks), `archive_sync/llm_enrichment/enrichment_orchestrator.py` (shared cache instance), `archive_sync/llm_enrichment/card_enrichment_runner.py` (optional `scan_cache` parameter), `archive_sync/llm_enrichment/enrich_runner.py` (optional `scan_cache` parameter), `archive_sync/llm_enrichment/document_text_extractor.py` (optional `scan_cache` parameter).
