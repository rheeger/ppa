...
That seed path is a **historical instance note**, not a default for `ppa setup`. New archives evaluate fail-closed on the current instance only and **do not inherit** `local_seed_living_corpus`. Knowledge-cache / 46-facet claims stay deferred. Analytics CLI/MCP stay pending until P10 / P04-D. `production_proven` stays false for fixture smokes.
