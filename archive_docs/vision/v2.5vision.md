...
Planning docs and implementation for Sections A–G are landed on branch `v2.5`, including D/E Phase 2, the A/B/C merge (`5737db3`), and the **scale hot-path commits** (`0c53d7e`…`3a90bc0`). What remains before **local full seed → v2.5** is: (1) slice hygiene that **deletes** historical marketing the same way inbound Gmail never writes those cards, and (2) executable updaters for **every data stream that is not a manual export**, proven on 1pct then 10pct. Only then Gate 5b on a seed copy. Arnold deploy/apply is deferred / out of current scope.
...
**Remaining sequence (slices first, then local full seed → v2.5):**

13. **Slice hygiene vault-remove + all live source updaters** on `.slices/1pct` then `.slices/10pct`. CCS-only apply is not cleaned. Every non-export stream must `--apply --max-items` and be findable. Manual exports stay import-only.
14. **Gate 5b re-proof** on a **full seed staging copy** (never the canonical seed). Catch-up + processors maintain on that copy.
15. **Local soak** + **Section F** readiness with real-run evidence (surfaces can go green on snapshots today).

Still-open scale holes **outside** those five engines: `embed_pending` is limit-N not `card_uid` filter. Dirty-UID rematerialize allowlist landed (`1c02e10`). Do not re-do the five landed I/O engines.

Arnold deploy/apply (H 6 / 6b / 7) is deferred / out of current scope. Do not start v3 packaging before the slice proofs and local seed remaining sequence above.
...
HEAD `3a90bc0` on branch `v2.5`. Canonical seed (never apply): `hf-archives-seed-20260307-235127`. Staging apply target: seed copy / `archive_seed_staging`. Slice ladder used `.slices/1pct` and a newly generated `.slices/10pct` (not a full-seed copy).

| Area                                                | Status                                                                                                                                                                                                                                                                                         |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Email promotion policy (A)                          | **Achieving** — real `EmailPromotionPolicy`                                                                                                                                                                                                                                                    |
| Corpus hygiene dry-run + apply/rollback tooling (B) | **Partial** — CCS apply is COPY and hides in Postgres; that is **not cleaned**. Remaining: vault-remove of suppressed/quarantine notes on 1pct/10pct (delete files, purge UIDs, write promotion ledger). Derived UIDs must be resolved. |
| Gmail classify-before-promote (C)                   | **Partial** — real adapter gate; full hydrate; file ledger                                                                                                                                                                                                                                     |
| Source updater **contract** (D Phase 1)             | **Achieving** — declarations, reports, snapshots                                                                                                                                                                                                                                               |
| Source updater **execution** (D Phase 2)            | **Partial** — runner is real; Gmail + Calendar proven on slices. Remaining: make **every live (non-export) adapter executable** and prove 1pct then 10pct. Manual exports stay `archive_sync` imports. |
| Processor **contract** (E Phase 1)                  | **Achieving** — declarations, plans, staleness helpers                                                                                                                                                                                                                                         |
| Processor **execution** (E Phase 2)                 | **Landed + Track A + dirty-only extract + UID rematerialize** (`1c02e10`). Remaining scale hole: `embed_pending` is limit-N not `card_uid` filter. |
...
| Validation/promotion runbook (H)                    | Local gates **0–5b done** for Gmail/Calendar + CCS hygiene. Scale ladder **landed**. Remaining: slice vault-remove + all live streams on 1pct/10pct, **then** seed-copy 5b + soak. Arnold deferred. |

`ppa maintain` can run source updaters and dirty-UID processors when those flags/keys are set. Fixture join proof (`5737db3`) is not live seed maintain.

### Path that unblocks local v2.5 (Arnold deferred)

```text
Scale hot paths — landed on 1pct + 10pct (3a90bc0)
  → slice hygiene vault-remove (leave 1pct/10pct cleaned)
  → live updaters for every non-export stream on 1pct then 10pct
  → H 5b re-proof on a full seed staging copy (never canonical seed)
  → catch-up + processors maintain on that copy
...
| ----------------------------------------- | ------------------------------------------------ | ---------------------------------------------------------------- |
| High-signal corpus                        | Self-hosters must not inherit a marketing mirror | A–C + slice vault-remove (CCS-only is not cleaned); Arnold B apply deferred |
| Scheduled maintain that refreshes sources | v3 maintainer container assumes this             | Gmail/Calendar landed; remaining live streams + seed-copy maintain still open |
| Incremental processors                    | Cannot afford full re-extract/re-embed           | E Phase 2 wired; dirty extract scoped; rebuild/embed still broad |
| Fail-closed readiness                     | Operators need an honest “is it healthy?”        | F surfaces landed; real-run evidence still open                  |
| Proven on one production machine          | Package stable behavior, not a snapshot          | Local seed remaining; Arnold deferred                            |
...
- Remaining slice work **deletes** suppressed/quarantine notes (thread + messages + attachments + derived-only cards), purges those UIDs, and writes the Gmail promotion ledger so inbound sync cannot recreate them. Same rule as inbound Gmail: those cards are never written.
...
- **Remaining (live coverage):** every data stream that refreshes itself must be executable in the runner and proven on 1pct then 10pct. A copied local store (iMessage snapshot, GitHub `gh` stage) is still live. A vendor CSV/XML/VCF/zip you drop in by hand is not.
...
- Phase 2: Gmail and Calendar updaters actually run adapters, commit cursors only after persisted side effects, and emit dirty UIDs into the processor path (landed + slice-proven).
- Every other live stream above is executable (`EXECUTABLE_ADAPTER_SOURCE_IDS`) and has a capped `--apply` proof on 1pct and 10pct.
...
- **Phase 2 (landed):** execute processors on dirty UIDs from D Phase 2; wire into `ppa maintain`; no full embedding/linker/LLM reruns by default. Dirty extract is UID-scoped (`35901b0`). Rebuild/embed still lack a true UID filter.
...
Phase 1 of A–G is landed. D/E Phase 2 runners are landed. The critical remaining edge is **slice vault-remove + remaining live updaters → H seed-copy 5b**. Section B Arnold apply stays part of H after updater proof.
...
| ------- | --------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| A       | Email Corpus Semantics            | Define active/suppressed/quarantine and `EmailPromotionPolicy`  | **Achieving**                                                                           | [section_a…](v2_5_execution_plans/section_a_email_corpus_semantics.plan.md)          |
| B       | Current Arnold Corpus Cleanup     | Clean existing email corpus using existing classification first | **Partial** — CCS apply landed; remaining is vault-remove on slices                     | [section_b…](v2_5_execution_plans/section_b_current_arnold_cleanup.plan.md)          |
| C       | Future Gmail Sync Promotion       | Classify before card promotion for new/changed Gmail threads    | **Partial** — adapter gate on; ledger must survive vault-remove                         | [section_c…](v2_5_execution_plans/section_c_future_gmail_sync_promotion.plan.md)     |
| D       | Source Updater Contract           | Freshness contract + **execute every live stream**              | Gmail/Calendar landed; remaining live adapters + 1pct/10pct proof                       | [section_d…](v2_5_execution_plans/section_d_source_updater_contract.plan.md)         |
| E       | Processor DAG                     | Refresh extract/enrich/embed/link from dirty inputs             | Phase 1 + Phase 2 **landed**; dirty extract scoped                                      | [section_e…](v2_5_execution_plans/section_e_processor_dag.plan.md)                   |
...
| G       | Validation Ladder + Rust Standard | Fixtures → slices → seed → Arnold rails                         | **Achieving** — scale ladder `3a90bc0`                                                  | [section_g…](v2_5_execution_plans/section_g_validation_ladder_rust_standard.plan.md) |
| H       | Validation + Promotion Runbook    | Prove live update + clean corpus through readiness              | Next: slice vault-remove + all live streams on 1pct/10pct, then seed-copy 5b. Arnold deferred | [section_h…](v2_5_execution_plans/section_h_validation_promotion_runbook.plan.md)    |
