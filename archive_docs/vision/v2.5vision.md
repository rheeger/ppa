...
**Living status:** [STATUS.md](../STATUS.md) (2026-09-09). Local close-out notes below stop at `5980464`. Ten-plan hardening ([PR 29](https://github.com/rheeger/ppa/pull/29)) and PRs 24–33 landed after that SHA and are part of v2.5, not a v2.75. When this file conflicts with STATUS, STATUS wins.
...
**v2.5-done is a local seed living archive, not Arnold promotion.** The canonical corpus on this machine is `/Users/rheeger/Archive/seed/hf-archives-seed-20260307-235127` (schema `ppa`). Arnold is **not** the intended long-term home of the corpus. It may be a remote HTTP MCP client of this machine. Some updaters need this Mac (or later Helga Pataki) — iMessage snapshots, Photos (parked), local Beeper, GitHub `gh`, Otter MCP, Google tokens. The ops model is: **run updaters where the secrets and devices live; the corpus lives here.**

v2.5 is not only hygiene and freshness. After the seed became the living corpus, the ten-plan hardening made retrieval and maintain correct at seed scale: trained hybrid search, conversation bursts, complete serving generations, access-bounded hops, independent instances, and honest analytics. That work stays in v2.5.
...
This is the retrieval and engine close of v2.5. A Markdown vault with a search box is not enough. The archive has to find the short answer in a long thread, refuse a half-written index, keep denied sources off neighbor hops, and answer subscription / trip / changes questions over the full eligible set without inventing a current plan.

Landed on main via [PR 29](https://github.com/rheeger/ppa/pull/29) (`product hardening: integrated ten-plan candidate`). Isolated acceptance passed on that SHA. `production_proven` stays false.

| Plan                       | Product meaning                                                                                                                                                                                                         |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P01 Retrieval fidelity     | Conversation bursts embed the short answer inside a long email or iMessage thread. Hybrid search fuses lexical and vector ranks (RRF). Semantic search uses trained IVF centroids. Ranking can prefer fresher evidence. |
| P02 Publication            | Changes are journaled. Query reads a complete serving generation. An incomplete publish cannot become `ACTIVE`.                                                                                                         |
| P03 Processor receipts     | One maintain path. Publish is the eligible checkpoint this run built. Failed revisions stay off the served watermark.                                                                                                   |
| P04 Acceptance             | Isolated product gate. Not a user feature. Not R0.                                                                                                                                                                      |
| P05 Access                 | `AccessContext` is applied before ranking and before neighbor expand. Denied sources do not leak through hops.                                                                                                          |
| P06 Engine ports           | CLI and MCP share one typed runtime. Burst attach and connector persist use the same path.                                                                                                                              |
| P07 Recovery               | Corrections keep a versioned story. Restore is not “rebuild and hope.”                                                                                                                                                  |
| P08 Connectors             | Account-isolated ingest and replay.                                                                                                                                                                                     |
| P09 Independent instances  | Two archives stay isolated through restart. Saved scopes are filters, not ACLs. Fixture `ppa setup` exists.                                                                                                             |
| P10 Evidence and analytics | Full-set typed query with coverage and freshness. Neighbor context. Bounded graph. `ppa analytics` / `archive_analytics` for subscriptions, trip costs, and changes-since.                                              |

PRs 24–28 made the serving index and nightly maintain path survive 1.4M cards. PRs 30–33 made person lookup honest across phone and email, and keyed embeddings by chunk text so rematerialize reuses paid vectors.

Nightly maintain is implemented and is **currently unloaded** on this machine. See [STATUS.md](../STATUS.md).
...
Local close-out HEAD was `5980464` on branch `v2.5`. Current main is later (ten-plan hardening + PRs 24–33). Canonical vault **is** `/Users/rheeger/Archive/seed/hf-archives-seed-20260307-235127` schema `ppa` on this machine. That seed **is** the apply target and the long-term corpus. “Never mutate the canonical seed” was a written-H constraint for an Arnold promotion path that was skipped and is **not** the product. Do not tell the next agent to copy the seed first. Arnold may be a remote client. It is not the home.
...
Scale hot paths — landed on 1pct + 10pct (3a90bc0)
  ...
  → Arnold is not the corpus home (it may be an HTTP MCP client)
  → ten-plan hardening (PR 29) + PRs 24–33 on main
```
...
- **Live (updater required):** Gmail, Calendar, iMessage (local Messages snapshot; the snapshot script needs Full Disk Access), Otter API, Documents (`file-libraries` live roots), Photos (`osxphotos`), Beeper SQLite (exclude iMessage/BlueBubbles — Helga-Pataki is a bridge, not a second iMessage source), Google Contacts API, Apple Contacts (`contacts:apple` via Contacts.app; the launchd GUI agent needs Automation permission for Contacts), Gmail correspondents API, GitHub (`gh` stage then ingest).
- **Export (no updater):** Copilot Finance CSV, LinkedIn Connections CSV, Notion people/staff CSV, Seed people, Apple Health XML, medical FHIR/CCD/Epic EHI/vaccine PDF, hand-dropped VCF. These stay `python -m archive_sync …` imports.
