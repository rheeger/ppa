...
Planning docs and implementation for Sections A–G are landed on branch `v2.5`, including D/E Phase 2, the A/B/C merge (`5737db3`), and the **scale hot-path commits** (`0c53d7e`…`3a90bc0`). What remains for **local full seed → v2.5** is Gate 5b re-proof on a real seed copy, catch-up + processors maintain, and soak. Arnold deploy/apply is deferred / out of current scope.
...
**Remaining sequence (local full seed → v2.5):**

13. **Gate 5b re-proof** on a **full seed staging copy** (never the canonical seed). Catch-up + processors maintain on that copy.
14. **Local soak** + **Section F** readiness with real-run evidence (surfaces can go green on snapshots today).

Still-open scale holes **outside** those five engines: rebuild has no UID allowlist; `embed_pending` is limit-N not `card_uid` filter. Do not re-do the five landed paths.

Arnold deploy/apply (H 6 / 6b / 7) is deferred / out of current scope. Do not start v3 packaging before the local seed remaining sequence above.
...
| Corpus hygiene dry-run + apply/rollback tooling (B) | **Partial** — real census/apply/rollback/retrieval filter; **apply is COPY** (10pct: 243,598 cards in 8.47s); census is one rust cache dump. Derived UIDs never resolved; no override store; embed/linker exclusion incomplete. Gate 5 staging apply 786k CCS + rollback **did happen** Jul 12 |
...
| Source updater **execution** (D Phase 2)            | **Landed + A/B/C merge + scale** — runner is real; Gmail presence/`quick_update` and Calendar invite/transcript indexes are vault-cache dumps (no markdown walk / `rglob`). Maintain still needs explicit source keys; cursors still list/page-token |
| Processor **contract** (E Phase 1)                  | **Achieving** — declarations, plans, staleness helpers                                                                                                          |
| Processor **execution** (E Phase 2)                 | **Landed + Track A + dirty-only extract** — stub executors closed; `uid_allowlist` no longer scans all `email_message`. Remaining scale holes: rebuild has no UID allowlist; `embed_pending` is limit-N not `card_uid` filter |
...
| Validation/promotion runbook (H)                    | Local gates **0–5b done**. Scale ladder **landed** (`3a90bc0`, 1pct + 10pct). Remaining local seed work: 5b re-proof on a **real seed copy** + maintain soak. Arnold (6 / 6b / 7) deferred / out of current scope |
...
| ----------------------------------------- | ------------------------------------------------ | -------------------------------------------------- |
| High-signal corpus                        | Self-hosters must not inherit a marketing mirror | A–C + local B apply; Arnold B apply deferred       |
| Scheduled maintain that refreshes sources | v3 maintainer container assumes this             | D/E Phase 2 landed; seed-copy maintain still open  |
...
- **Phase 2 (landed):** execute updaters by running existing adapters under the contract; commit batches/cursors; emit dirty UIDs; integrate into `ppa maintain`. Snapshots alone do not count as freshness. Gmail/Calendar presence and invite indexes are cache-backed as of `0bdbd48` / `9139c58`.
...
| B       | Current Arnold Corpus Cleanup     | Clean existing email corpus using existing classification first | **Partial** — tooling + Gate 5 + COPY scale (1/10pct) | [section_b…](v2_5_execution_plans/section_b_current_arnold_cleanup.plan.md)          |
| C       | Future Gmail Sync Promotion       | Classify before card promotion for new/changed Gmail threads    | **Partial** — adapter gate; enable on seed copy    | [section_c…](v2_5_execution_plans/section_c_future_gmail_sync_promotion.plan.md)     |
| D       | Source Updater Contract           | Freshness contract + **execute adapters**                       | Phase 1 + Phase 2 **landed**; Gmail/Calendar cache indexes | [section_d…](v2_5_execution_plans/section_d_source_updater_contract.plan.md)         |
...
| H       | Validation + Promotion Runbook    | Prove live update + clean corpus through readiness              | Local 0–5b + scale landed; **next** is 5b re-proof on seed copy + soak. Arnold deferred | [section_h…](v2_5_execution_plans/section_h_validation_promotion_runbook.plan.md)    |
