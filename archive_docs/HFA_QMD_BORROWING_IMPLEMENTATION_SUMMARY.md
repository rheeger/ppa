...
- New module: `archive_cli/retrieval_pipeline.py` (merge, fuse, rank, score breakdowns, `PIPELINE_VERSION`).
...
- New module: `archive_cli/query_planner.py`.
...
- New module: `archive_cli/reranker.py`: `none`, `heuristic` (token overlap), `model` (currently same as heuristic until a real API is wired).
