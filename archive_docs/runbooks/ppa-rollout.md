...
**Rule:** for any SSH sanity check (`psql`, `python -c 'import archive_cli…'`, etc.), **`set -a; . /home/arnold/openclaw/.env; set +a`** on the remote shell first, then use **`$ARCHIVE_INDEX_DSN`**, **`$ARCHIVE_INDEX_SCHEMA`**, **`$HFA_VAULT_PATH`**, **`$HFA_LIB_PATH`** as exported there.
...
ssh arnold@192.168.50.27 'set -a; . /home/arnold/openclaw/.env; set +a; sudo -u archive env \
  ...
  /home/arnold/openclaw/venv/bin/python -c "import archive_cli.server; print(\"server_import_ok\")"'
```
