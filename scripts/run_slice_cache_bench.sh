...
# health-check --manifest tests/slice_manifest.json (override with PPA_SLICE_MANIFEST).
...
run_verify_slice() {
  ...
  if [[ "$_BENCH_BOOTSTRAP_DONE" -eq 0 ]]; then
    /usr/bin/time -p .venv/bin/python -m archive_mcp --log-file "$LOGDIR/bootstrap-${tag}.log" bootstrap-postgres
