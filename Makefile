...
# Same vault/schema as archive_scripts/run-local-seed-mcp.sh (derived index in Postgres schema archive_seed)
...
dump-schema:
	@DUMP_SCHEMA=$(DUMP_SCHEMA) ARCHIVE_DUMP_OUT=$(PPA_DUMP_OUT) \
		...
		bash archive_scripts/dump-schema-local.sh
...
# Stream archive_seed custom dump to Arnold (see archive_scripts/stream-seed-dump-to-arnold.sh).
...
dump-seed-schema-stream-arnold:
	@bash archive_scripts/stream-seed-dump-to-arnold.sh
...
pipe-restore-seed-arnold:
	@bash archive_scripts/stream-seed-pipe-restore-arnold.sh
...
scp-restore-seed-arnold:
	@PG_RESTORE_JOBS=$(PG_RESTORE_JOBS) ARCHIVE_DUMP_OUT=$(PPA_DUMP_OUT) ARCHIVE_DUMP_ARTIFACT=$(PPA_DUMP_ARTIFACT) \
		...
		bash archive_scripts/scp-restore-seed-arnold.sh
...
watch-scp-restore-continue-hfa:
	@POLL_INTERVAL_SEC=$(POLL_INTERVAL_SEC) POLL_MAX_WAIT_SEC=$(POLL_MAX_WAIT_SEC) bash archive_scripts/poll-scp-restore-and-continue-hfa.sh
...
# only the most recent run by default; override with KEEP=N. See
# `archive_docs/runbooks/embedding-recovery-cache.md`.
embed-cache-rotate:
	$(PYTHON) -m archive_cli embed-cache-rotate $(if $(KEEP),--keep $(KEEP)) $(if $(DRY_RUN),--dry-run)
...
benchmark-rebuild:
	@PPA_INDEX_DSN="$$( $(LOCAL_PPA_INDEX_DSN_CMD) )"; \
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	$(PYTHON) -m archive_cli benchmark-rebuild \
		--vault "$(PPA_PATH)" \
...
benchmark-seed-links:
	@PPA_INDEX_DSN="$$( $(LOCAL_PPA_INDEX_DSN_CMD) )"; \
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	$(PYTHON) -m archive_cli benchmark-seed-links \
		--vault "$(PPA_PATH)" \
		--schema "$(PPA_INDEX_SCHEMA)" \
		--profile "$(PPA_BENCHMARK_PROFILE)" \
		$(if $(PPA_REBUILD_WORKERS),--workers $(PPA_REBUILD_WORKERS)) \
...
# Phase 2.9 — archive_crate JSON benchmarks (``cd archive_crate && maturin develop``; set PPA_BENCHMARK_VAULT to override seed).
benchmark-archive-crate-tier1:
	PPA_BENCHMARK_VAULT=$(PPA_SEED_VAULT) $(PYTHON) archive_scripts/benchmark-archive_crate-tier1.py

# Fails with exit 2 if walk/cache speedups are below defaults (8×) when note_count >= PPA_TIER1_ENFORCE_MIN_NOTES (default 500). See archive_docs/reports/archive_crate-benchmark-tier1-baseline.json
benchmark-archive-crate-tier1-enforce:
	PPA_BENCHMARK_VAULT=$(PPA_SEED_VAULT) $(PYTHON) archive_scripts/benchmark-archive_crate-tier1.py --enforce

benchmark-archive-crate-tier2:
	PPA_BENCHMARK_VAULT=$(PPA_SEED_VAULT) $(PYTHON) archive_scripts/benchmark-archive_crate-tier2.py

benchmark-archive-crate-entity:
	PPA_BENCHMARK_VAULT=$(PPA_SEED_VAULT) $(PYTHON) archive_scripts/benchmark-archive_crate-entity-resolution.py
...
migrate-dry-run:
	...
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	PPA_INDEX_SCHEMA=$(PPA_INDEX_SCHEMA) \
	$(PYTHON) -m archive_cli migrate --dry-run
...
migration-status:
	...
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	PPA_INDEX_SCHEMA=$(PPA_INDEX_SCHEMA) \
	$(PYTHON) -m archive_cli migration-status
...
smoke-queries:
	...
	bash archive_scripts/ppa-smoke-test.sh

arnold-smoke:
	ssh arnold@192.168.50.27 'cd /srv/ppa && ./archive_scripts/ppa-smoke-test.sh'

smoke: install bootstrap-postgres rebuild-indexes index-status embed-pending smoke-queries

test-unit:
	$(PYTHON) -m pytest archive_tests/ -v --tb=short -m "not integration and not slow"

# Phase 2.9 — archive_crate (Rust + PyO3). Requires: pip install maturin
export-materializer-registry:
	$(PYTHON) archive_scripts/export_materializer_registry.py

build-rust:
	cd archive_crate && maturin develop --release

test-rust: build-rust
	$(PYTHON) -m pytest archive_tests/test_archive_crate_smoke.py archive_tests/test_archive_crate_walk.py archive_tests/test_archive_crate_hasher.py archive_tests/test_archive_crate_frontmatter.py archive_tests/test_archive_crate_cache.py archive_tests/test_archive_crate_scanner.py archive_tests/test_archive_crate_materializer_chunker.py archive_tests/test_ppa_engine_vault_cache.py -v
	cd archive_crate && cargo test

# Release-mode Rust tests (no Criterion benches in-tree yet).
benchmark-rust: build-rust
	cd archive_crate && cargo test --release

# Phase 2.75 Step 1b — requires local Ollama + model (e.g. gemma4:31b). Non-strict: exits 0 if Ollama down.
ollama-llm-smoke:
	$(PYTHON) archive_scripts/ollama_llm_smoke.py

# Phase 2.75 Step 6b — triage + extract one thread (human review). Example: --vault archive_tests/fixtures --list-threads
llm-enrichment-6b-smoke:
	@echo "Run: $(PYTHON) archive_scripts/llm_enrichment_step6b_smoke.py --vault <VAULT> --list-threads"
	@echo "Then: $(PYTHON) archive_scripts/llm_enrichment_step6b_smoke.py --vault <VAULT> --thread-id <ID>"
...
test-integration:
	...
	PPA_EMBEDDING_MODEL=archive-hash-dev \
	PPA_EMBEDDING_VERSION=1 \
	$(PYTHON) -m pytest archive_tests/ -v --tb=short -m "integration"
...
test-slice:
	@PPA_INDEX_DSN="$$( $(LOCAL_PPA_INDEX_DSN_CMD) )"; \
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	$(PYTHON) -m archive_cli slice-seed \
		--config archive_tests/slice_config.json \
...
test-slice-smoke:
	...
	PPA_INDEX_DSN="$$( $(LOCAL_PPA_INDEX_DSN_CMD) )"; \
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	$(PYTHON) -m archive_cli --log-file logs/ppa-slice-smoke.log slice-seed \
		--config archive_tests/slice_config.smoke.json \
		--output /tmp/ppa-test-slice-smoke \
		--source-vault "$(PPA_BENCHMARK_SOURCE_VAULT)" \
		--progress-every 2000

# Local 1% / 5% / 10% slices under $(SLICES_LOCAL_DIR)/ (gitignored); configs archive_tests/slice_config.{1,5,10}pct.json
slice-local-1pct:
	@mkdir -p $(SLICES_LOCAL_DIR)/1pct
	$(PYTHON) -m archive_cli slice-seed \
		--config archive_tests/slice_config.1pct.json \
...
slice-local-5pct:
	...
	$(PYTHON) -m archive_cli slice-seed \
		--config archive_tests/slice_config.5pct.json \
...
slice-local-10pct:
	...
	$(PYTHON) -m archive_cli slice-seed \
		--config archive_tests/slice_config.10pct.json \
		--output $(SLICES_LOCAL_DIR)/10pct \
		--source-vault "$(PPA_BENCHMARK_SOURCE_VAULT)" \
		--progress-every 30000

slice-local-identity-1pct:
	@mkdir -p logs $(SLICES_LOCAL_DIR)/identity-1pct logs/identity-repair
	$(PYTHON) -m archive_cli --log-file logs/slice-identity-1pct.log slice-seed \
		--config archive_tests/slice_config.identity-1pct.json \
		--output $(SLICES_LOCAL_DIR)/identity-1pct \
		--source-vault "$(PPA_BENCHMARK_SOURCE_VAULT)" \
		--progress-every 5000
	$(PYTHON) archive_tests/fixtures/identity-repair/overlay_into_slice.py \
		--slice $(SLICES_LOCAL_DIR)/identity-1pct \
		--source-vault "$(PPA_BENCHMARK_SOURCE_VAULT)" \
		--inventory logs/identity-repair/s0-slice-inventory.json
...
test-slice-verify:
	...
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	PPA_INDEX_SCHEMA=archive_test_slice \
	$(PYTHON) -m archive_cli health-check --manifest archive_tests/slice_manifest.json --report-format both
...
test-slice-verify-smoke:
	...
	PPA_EMBEDDING_MODEL=archive-hash-dev \
	PPA_EMBEDDING_VERSION=1 \
	$(PYTHON) -m archive_cli --log-file logs/ppa-rebuild-smoke.log rebuild-indexes && \
	PPA_PATH=/tmp/ppa-test-slice-smoke \
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	PPA_INDEX_SCHEMA=archive_test_slice_smoke \
	$(PYTHON) -m archive_cli health-check --manifest archive_tests/slice_manifest.smoke.json --report-format both \
		--report-dir logs
...
verify-incremental:
	...
	PPA_EMBEDDING_MODEL=archive-hash-dev \
	PPA_EMBEDDING_VERSION=1 \
	$(PYTHON) -m pytest archive_tests/test_rebuild_incremental.py -v --tb=short

benchmark-1pct:
	@PPA_INDEX_DSN="$$( $(LOCAL_PPA_INDEX_DSN_CMD) )"; \
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	$(PYTHON) -m archive_cli benchmark --slice-percent 1 --output /tmp/bench-results/

benchmark-5pct:
	@PPA_INDEX_DSN="$$( $(LOCAL_PPA_INDEX_DSN_CMD) )"; \
	PPA_INDEX_DSN="$$PPA_INDEX_DSN" \
	$(PYTHON) -m archive_cli benchmark --slice-percent 5 --output /tmp/bench-results/
...
maintain-dry-run:
	...
	PPA_INDEX_SCHEMA=$(PPA_INDEX_SCHEMA) \
	PPA_PATH=$(PPA_PATH) \
	$(PYTHON) -m archive_cli maintain --dry-run

# Nightly 2am local-seed maintain (launchd). Does not load unless you run this target.
# See archive_docs/vision/v2_5_execution_plans/README.md (Nightly maintain).
install-nightly-maintain:
	$(PYTHON) archive_scripts/ppa-maintain-nightly.py --install
...
clean-phase3-derived:
	bash archive_scripts/clean-phase3-derived-dirs.sh $(if $(APPLY),--apply) "$(PPA_PATH)"

# Same cleanup for default Makefile slice outputs (paths only; no-op if missing).
clean-phase3-derived-slices:
	bash archive_scripts/clean-phase3-derived-dirs.sh $(if $(APPLY),--apply) /tmp/ppa-test-slice-smoke /tmp/ppa-test-slice

clean-phase3-derived-local-slices:
	bash archive_scripts/clean-phase3-derived-dirs.sh $(if $(APPLY),--apply) \
		"$(CURDIR)/$(SLICES_LOCAL_DIR)/1pct" \
		"$(CURDIR)/$(SLICES_LOCAL_DIR)/5pct" \
		"$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct"

# Remove /tmp slice vaults, extract staging, bench dirs (see archive_scripts/clean-ppa-machine-artifacts.sh --help).
clean-ppa-machine-artifacts:
	bash archive_scripts/clean-ppa-machine-artifacts.sh

clean-ppa-machine-artifacts-dry-run:
	bash archive_scripts/clean-ppa-machine-artifacts.sh --dry-run

# Deletes .slices/{1pct,5pct,10pct} only (multi-GB). Seed vault untouched.
clean-ppa-local-slices:
	bash archive_scripts/clean-ppa-machine-artifacts.sh --remove-local-slices

# Extract from local stratified slices (see .slices/README.md). Cleans Phase-3 dirs on that slice first.
# Slice vaults are regenerable so we pass --apply unconditionally; production vault path would still be refused.
extract-emails-1pct-slice:
	bash archive_scripts/clean-phase3-derived-dirs.sh --apply "$(CURDIR)/$(SLICES_LOCAL_DIR)/1pct"
	rm -rf _artifacts/_staging-1pct/
	PPA_PATH="$(CURDIR)/$(SLICES_LOCAL_DIR)/1pct" $(PYTHON) -m archive_cli --log-file logs/extract-1pct-slice.log extract-emails \
		--staging-dir _artifacts/_staging-1pct/ --workers 8 --full-report

extract-emails-5pct-slice:
	bash archive_scripts/clean-phase3-derived-dirs.sh --apply "$(CURDIR)/$(SLICES_LOCAL_DIR)/5pct"
	rm -rf _artifacts/_staging-5pct/
	PPA_PATH="$(CURDIR)/$(SLICES_LOCAL_DIR)/5pct" $(PYTHON) -m archive_cli --log-file logs/extract-5pct-slice.log extract-emails \
		--staging-dir _artifacts/_staging-5pct/ --workers 8 --full-report

extract-emails-10pct-slice:
	bash archive_scripts/clean-phase3-derived-dirs.sh --apply "$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct"
	rm -rf _artifacts/_staging-10pct/
	PPA_PATH="$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct" $(PYTHON) -m archive_cli --log-file logs/extract-10pct-slice.log extract-emails \
...
extraction-quality-reports:
	$(PYTHON) archive_scripts/generate_extraction_quality_report.py --staging-dir _artifacts/_staging-1pct --vault "$(CURDIR)/$(SLICES_LOCAL_DIR)/1pct" --label 1pct --out archive_docs/reports/extraction-quality/1pct.md --problem-samples 8 --clean-samples 4
	$(PYTHON) archive_scripts/generate_extraction_quality_report.py --staging-dir _artifacts/_staging-5pct --vault "$(CURDIR)/$(SLICES_LOCAL_DIR)/5pct" --label 5pct --out archive_docs/reports/extraction-quality/5pct.md --problem-samples 8 --clean-samples 4
	$(PYTHON) archive_scripts/generate_extraction_quality_report.py --staging-dir _artifacts/_staging-10pct --vault "$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct" --label 10pct --out archive_docs/reports/extraction-quality/10pct.md --problem-samples 8 --clean-samples 4

# EDL Phase 1–2: taxonomy + per-year template samples (set DOMAIN=, optional CATEGORY=, PPA_PATH=).
sender-census:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli sender-census --domain $(DOMAIN) --sample 100

template-sampler:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli template-sampler \
...
sender-census-slice-smoke:
	@test -d "$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct" || { echo "Missing $(SLICES_LOCAL_DIR)/10pct — run: make slice-local-10pct"; exit 1; }
	PPA_PATH="$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct" $(PYTHON) -m archive_cli sender-census --domain $(DOMAIN) --sample 50

template-sampler-slice-smoke:
	@test -d "$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct" || { echo "Missing $(SLICES_LOCAL_DIR)/10pct — run: make slice-local-10pct"; exit 1; }
	PPA_PATH="$(CURDIR)/$(SLICES_LOCAL_DIR)/10pct" $(PYTHON) -m archive_cli template-sampler \
...
step-11a-template-samplers:
	PER_YEAR="$(STEP_11A_PER_YEAR)" $(CURDIR)/archive_scripts/run_step_11a_template_samplers.sh
...
sender-census-seed:
	...
	PPA_PATH="$(PPA_SEED_VAULT)" $(PYTHON) -m archive_cli sender-census \
...
step-11a-template-samplers-seed:
	@test -d "$(PPA_SEED_VAULT)" || { echo >&2 "Missing PPA_SEED_VAULT: $(PPA_SEED_VAULT)"; exit 1; }
	RUN_ON_SEED=1 SEED_VAULT="$(PPA_SEED_VAULT)" PER_YEAR="$(STEP_11A_PER_YEAR_SEED)" $(CURDIR)/archive_scripts/run_step_11a_template_samplers.sh
...
step-11d-slice-yield-report:
	...
	$(PYTHON) archive_scripts/step_11d_slice_yield_report.py --vault "$(STEP_11D_VAULT)"
...
extract-emails-slice-smoke: clean-phase3-derived-slices
	PPA_PATH=/tmp/ppa-test-slice-smoke $(PYTHON) -m archive_cli extract-emails \
...
extract-emails-slice-full: clean-phase3-derived-slices
	PPA_PATH=/tmp/ppa-test-slice $(PYTHON) -m archive_cli extract-emails \
		--staging-dir /tmp/ppa-slice-full-extract-staging --workers 4 --full-report

extract-emails-staging:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli extract-emails \
...
extract-dry-run:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli extract-emails --dry-run
...
enrich-emails-staging:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli --log-file logs/enrich-emails.log enrich-emails \
...
enrich-emails-gemini:
	...
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli --log-file logs/enrich-emails-gemini.log enrich-emails \
		--staging-dir _artifacts/_staging-llm/ --cache-db _artifacts/_enrichment_cache.db --progress-every 25 --full-report \
...
enrich-cards-gemini-1pct:
	GEMINI_API_KEY=$$(op read '$(PPA_GEMINI_API_KEY_OP_REF)') \
	PPA_PATH="$(CURDIR)/$(SLICES_LOCAL_DIR)/1pct" $(PYTHON) -m archive_cli --log-file logs/enrich-cards-gemini-1pct.log enrich-cards \
...
enrich-cards-gemini-1pct-preview:
	GEMINI_API_KEY=$$(op read '$(PPA_GEMINI_API_KEY_OP_REF)') \
	PPA_PATH="$(CURDIR)/$(SLICES_LOCAL_DIR)/1pct" $(PYTHON) -m archive_cli --log-file logs/enrich-cards-gemini-1pct-preview.log enrich-cards \
		--workflow email_thread \
...
build-enrichment-benchmark:
	$(PYTHON) archive_scripts/build_enrichment_benchmark.py \
...
step8b-review-packet:
	$(PYTHON) archive_scripts/prepare_step8b_human_review.py --ground-truth "$(STEP8B_GROUND_TRUTH)" --out _artifacts/_benchmark/step8b_review_packet.md

# Phase 2.75 Step 9 — LLM vs ground truth (Ollama). Gemma 4 only: gemma4:e2b,e4b,26b,31b (see archive_docs/gemma4-local-models.md).
...
run-enrichment-benchmark:
	$(PYTHON) archive_scripts/run_enrichment_benchmark.py \
...
aggregate-benchmark-results:
	$(PYTHON) archive_scripts/aggregate_benchmark_results.py --results-dir "$(STEP9_OUT)"

staging-report:
	$(PYTHON) -m archive_cli staging-report --staging-dir _artifacts/_staging/

promote-staging:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli promote-staging --staging-dir _artifacts/_staging/

promote-staging-dry-run:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli promote-staging --staging-dir _artifacts/_staging/ --dry-run

resolve-entities:
	PPA_PATH=$(PPA_PATH) $(PYTHON) -m archive_cli resolve-entities
